...
class KittyBaseballGame: SKScene, SKPhysicsContactDelegate {
    ...
    var Baseball_EndPoint: CGPoint = CGPoint.zero
    ...
    func Status_Natsume_Kyousuke(){
        if Natsume_Kyousuke.attribute.status == GamePeople.Natsume_Kyousuke_Status.NK_Run.hashValue{
        ...
        case GamePeople.Natsume_Kyousuke_Status.NK_Run.hashValue:
            ...
            if(Natsume_Kyousuke_View.hasActions() == false){
                let RunPath = UIBezierPath()
                RunPath.moveToPoint(Natsume_Kyousuke_View.position)
                if(Baseball_EndPoint.x > Natsume_Kyousuke_View.position.x){
                    RunPath.addLineToPoint()
                }
                else{
                    RunPath.addLineToPoint()
                }
                Natsume_Kyousuke_View.runAction(SKAction.followPath(RunPath.CGPath, asOffset: false, orientToPath: false, speed: 30), completion: { () -> Void in
                    self.Natsume_Kyousuke.attribute.status = GamePeople.Natsume_Kyousuke_Status.NK_Static.hashValue
                    self.Natsume_Kyousuke_View.removeAllActions()
                })
            }
    ...
    func Baseball_Return(Baseball_Number: Int,contact: CGPoint){
        ...
        Baseball_EndPoint = BallPath.currentPoint
        ...
        Baseball[Baseball_Number].Baseball_Unit.runAction(SKAction.speedTo(0, duration: 5), completion: { () -> Void in
            self.Baseball_Static(Baseball_Number)
            self.Baseballfield.removeChildrenInArray([BallTrackPath])
        })
    }
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        if ContactUnit.categoryBitMask == Collision.PeopleFront && Baseball[0].Baseball_Status == GameObject.Baseball_Status.B_Cast{
            ...
            Baseball[0].Baseball_Image.hidden = true
            Baseball[0].Baseball_Shadow.hidden = true
            Baseball[0].Baseball_Status = .B_Static
        }
    }
